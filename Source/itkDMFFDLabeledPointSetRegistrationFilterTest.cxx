/*=========================================================================
  ...
  Module:    $RCSfile: itkDMFFDLabeledPointSetRegistrationFilterTest.cxx,v $
  Language:  C++
  Date:      $Date: $
  Version:   $Revision: $
...
int itkDMFFDLabeledPointSetRegistrationFilterTest( itk::CommandLineParser *parser )
  ...
  if( transformationOption->GetNumberOfParameters() > 3 )
    ...
    try
      {
      reader->Update();
      registrationFilter->SetInitialDeformationFieldControlPointLattice(
        reader->GetOutput() );
      }
    catch( ... )
      {
  ...
  registrationFilter->SetPointSetSigma( parser->template Convert<float>(
  ...
  if( optimizationOption->GetNumberOfParameters() > 1 )
    ...
    registrationFilter->SetLineSearchMaximumStepSize( parser->template
      Convert<float>( optimizationOption->GetParameter( 1 ) ) );
    }
  if( optimizationOption->GetNumberOfParameters() > 2 )
    {
    std::vector<typename RegistrationFilterType::RealType> relaxFactors =
      parser->template ConvertVector<typename RegistrationFilterType::RealType>(
      optimizationOption->GetParameter( 2 ) );
    typename RegistrationFilterType::ResizableRealArrayType
      relaxationFactors;
    relaxationFactors.SetSize( numIterations.size() );
    if( relaxFactors.size() != numIterations.size() )
      {
      relaxationFactors.Fill( relaxFactors[0] );
      }
    else
      {
      for( unsigned int i = 0; i < relaxFactors.size(); i++ )
        {
        relaxationFactors[i] = relaxFactors[i];
        }
      }
    registrationFilter->SetRelaxationFactor( relaxationFactors );
    }
...
//    typename RegistrationFilterType::RealImageType>
//    ControlPointLatticeWriterType;
//

//  itk::OStringStream buf;
//  buf << "_" << registrationFilter->GetSplineOrder();
//
...
//  cpwriter->SetInput(
//    registrationFilter->GetTotalDeformationFieldControlPointLattice() );
//  cpwriter->Update();
  ...
  bspliner->SetDirection( registrationFilter->GetDirection() );
  bspliner->Update();
...
void InitializeCommandLineOptions( itk::CommandLineParser *parser )
{
  ...
  {
  std::string description =
    std::string( "[splineOrder,meshResolution," ) +
    std::string( "<domainImage>,<initialControlPointLattice>]" );
  ...
  std::string description =
    ...
    std::string( "<lineSearchMaximumStepSize>,<relaxationFactor(s)>]" );
